...
class _PreviewPanelState extends State<PreviewPanel> {
  ...
  bool _loading = false;
  String? _error;
  // PdfController for inline PDF preview (null until a PDF is selected).
  ...
  Widget _buildPreview() {
    ...
    if (item.isPdf) {
      if (_pdfController != null) {
        ...
        return Column(
          ...
          children: [
            ...
            if (_pdfPageCount > 1)
              Padding(
                ...
                child: Row(
                  children: [
                    Text(
                      'Page $_pdfCurrentPage${_pdfPageCount > 0 ? ' of $_pdfPageCount' : ''}',
                      style: TextStyle(
                    ...
                    IconButton(
                      ...
                      constraints: const BoxConstraints(
                          minWidth: 32, minHeight: 32),
                      onPressed: _pdfCurrentPage < _pdfPageCount
                          ? () {
            ...
            // Inline PDF view — constrained height so the panel stays scrollable
            SizedBox(
              height: 400,
              child: PdfView(
                ...
                onDocumentError: (error) {
                  // pdfium failed — switch to text fallback
                  if (mounted) {
                    setState(() {
                      _pdfController?.dispose();
                      _pdfController = null;
                      _content =
                          DocConverter.extractPdfText(item.path) ?? '';
                    });
                  }
                },
                onPageChanged: (page) {
                ...
                builders: PdfViewBuilders<DefaultBuilderOptions>(
                  ...
                  documentLoaderBuilder: (ctx) => const Center(
      ...
      // No controller means pdfium failed — show extracted text
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Text preview (native PDF rendering unavailable)',
                    style: TextStyle(
                        color: cs.onSurfaceVariant, fontSize: 11),
                  ),
                ),
