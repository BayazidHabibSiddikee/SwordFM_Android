...
class _TrashScreenState extends State<TrashScreen> {
  ...
  Future<void> _restore(FileItem item) async {
    final originalPath = await _originalPath(item.path);
    try {
  ...
  Future<String> _originalPath(String trashPath) async {
    ...
    // On Android AppPaths.trash is empty — use the app-local trash dir.
    return p.join(p.dirname(await AppPaths.trashDir), possibleBase);
  }
  ...
  Future<void> _emptyTrash() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        backgroundColor: OneDarkColors.bg,
        title: Text('Empty Trash?', style: TextStyle(color: OneDarkColors.fg)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Cancel'),
          ...
          TextButton(
            onPressed: () => Navigator.pop(context, true),
            child: Text('Empty', style: TextStyle(color: OneDarkColors.red)),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: OneDarkColors.bg,
      appBar: AppBar(
        title: Text('Trash', style: TextStyle(color: OneDarkColors.fg)),
        backgroundColor: OneDarkColors.bgDark,
        foregroundColor: OneDarkColors.fg,
        actions: [
          if (_items.isNotEmpty)
            IconButton(
              icon: Icon(Icons.delete_sweep, color: OneDarkColors.red),
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  trailing: Row(
                    ...
                    children: [
                      IconButton(
                        icon: Icon(Icons.restore, color: OneDarkColors.cyan),
