...
class _PdfReaderScreenState extends State<PdfReaderScreen>
  ...
  void _preloadAround(int page) {
    for (
      int i = (page - 1).clamp(1, _totalPages);
  ...
  Future<void> _loadPrefs() async {
    ...
    setState(() {
      _bookmarks.addAll(
        bm.map((s) => int.tryParse(s) ?? 0).where((x) => x > 0),
      );
      for (final raw in ms) {
        try {
          _marks.add(PdfMark.fromJson(jsonDecode(raw)));
        } catch (_) {}
      }
    });
  }
  ...
  Future<void> _saveMarks() async {
    ...
    await p.setStringList(
      _markKey,
      _marks.map((m) => jsonEncode(m.toJson())).toList(),
    );
  }
  ...
  void _runSearch(String query) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (line.toLowerCase().contains(q)) {
        ...
        final pageEstimate = ((charOffset / _charsPerPage) + 1).round().clamp(
          1,
          _totalPages > 0 ? _totalPages : 9999,
        );
        results.add(
          _SearchResult(
            snippet: snippet,
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // ── Marking hint ────────────────────────────────────────────────
          if (_activeTool != MarkTool.none)
            Positioned(bottom: 32, left: 16, child: _buildMarkHint()),

          // ── Settings panel ──────────────────────────────────────────────
          if (_settingsOpen)
            Positioned(bottom: 0, left: 0, right: 0, child: _buildSettings()),
        ],
      ),
    );
  }
          ...
          children: [
            ...
            if (_marks.any((m) => m.page == _currentPage))
              IconButton(
                ...
                onPressed: () {
                  setState(
                    () => _marks.removeWhere((m) => m.page == _currentPage),
                  );
                  _saveMarks();
  ...
  Widget _navBtn(IconData icon, bool enabled, VoidCallback onTap) => IconButton(
...
class _LiveMarkPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (tool == MarkTool.highlight) {
      ...
      canvas.drawLine(
        Offset(dragStart!.dx, y),
        Offset(dragCurrent!.dx, y),
        Paint()
